...
//      call.
//
// Компрессор LZSS требует 54301 динамической памяти
// --------------------------------------------------------------------------
T_compress_size Compress_lzss(compress_data_t *pInput, compress_data_t *pOutput)
{
  ...
  if (_nCodeBufIdx > 1)
  {
    for (_nIdx = 0; _nIdx < _nCodeBufIdx; _nIdx++) cio_Put_char(pOutput,_aryCodeBuf[_nIdx]);
  }
...
//  Notes:
//      None
//  Декомпрессор LZSS требует 4331 байт динамической памяти
//--------------------------------------------------------------------------
T_compress_size Uncompress_lzss(compress_data_t *pInput, compress_data_t *pOutput)
{
  ...
  while (1)
  {
    ...
    if (flags & 1)
    {
      if ((_ch = cio_Get_char(pInput)) == EOF) break;
