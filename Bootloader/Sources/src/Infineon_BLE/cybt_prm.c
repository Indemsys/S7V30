...
bool cybt_prm_download(cybt_prm_cback_t *p_cb,const uint8_t  *p_patch_buf,uint32_t patch_buf_len,uint32_t address,uint8_t  format_type)
{
  ...
  wiced_bt_dev_vendor_specific_command(cybt_prm_cb.opcode,0,NULL,cybt_prm_command_complete_cback);
...
static void cybt_prm_command_complete_cback(wiced_bt_dev_vendor_specific_command_complete_params_t *p_param)
{
  ...
  {
    if (cybt_prm_cb.p_cb) (cybt_prm_cb.p_cb)(CYBT_PRM_STS_ABORT);
    return;
  }
  ...
  case CYBT_PRM_ST_INITIALIZING:
    ...
    {
      if (cybt_prm_cb.p_cb) (cybt_prm_cb.p_cb)(CYBT_PRM_STS_CONTINUE);
    }
    else
    {
  ...
  case CYBT_PRM_ST_LAUNCHING_RAM:
    ...
    if (cybt_prm_cb.p_cb) (cybt_prm_cb.p_cb)(CYBT_PRM_STS_COMPLETE);
...
static void cybt_prm_send_next_patch(void)
{
  ...
  if (cybt_prm_cb.format_type == CYBT_PRM_FORMAT_HCD)
  {
    ...
    wiced_bt_dev_vendor_specific_command(vsc_command,(uint8_t)len,p,cybt_prm_command_complete_cback);
  ...
  else
  {
    ...
    if (len > CYBT_MAX_PRM_LEN) len = CYBT_MAX_PRM_LEN;
    ...
    wiced_bt_dev_vendor_specific_command(HCI_VSC_WRITE_RAM,(uint8_t)(len+4),write_ram_vsc,cybt_prm_command_complete_cback);
...
**
*******************************************************************************/
bool cybt_prm_load_data(uint8_t  *p_patch_data,uint16_t patch_data_len)
{
  /* Only load data if loading minidri or data piece is done */
  if (cybt_prm_cb.state != CYBT_PRM_ST_INITIALIZING_DONE && cybt_prm_cb.state != CYBT_PRM_ST_LOADING_DATA_DONE)
  {
    return false;
  }
...
static void cybt_prm_launch_ram_internal(void)
{
  ...
  wiced_bt_dev_vendor_specific_command(HCI_VSC_LAUNCH_RAM,4,write_launch_ram_vsc,cybt_prm_command_complete_cback);
}
