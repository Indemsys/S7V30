  ...
  #define   EXT_REF_VOLTAGE                 (3.29f)                            // Уровень напряжения откносительно которого ведутся измерения АЦП
...
uint32_t    ADS7028_read_byte(uint8_t n, uint8_t addr, uint8_t *val);
uint32_t    ADS7028_write_byte(uint8_t n, uint8_t addr, uint8_t val);
