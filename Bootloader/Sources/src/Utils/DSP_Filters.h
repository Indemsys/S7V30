...
typedef struct
{
   uint32_t  k;    // Степеь коэффициента фильтра - n. Коэффициент выражается в виде 2^n , т.е. является степенью двойки для того чтобы можно было применять сдвиги вместо делений и умножений
   uint32_t  v1;   // Сохраненное значение
   int8_t    init; // Флаг инициализации.  Если 0, то фильтрация начинается от текущего входного значения, иначе от нуля
