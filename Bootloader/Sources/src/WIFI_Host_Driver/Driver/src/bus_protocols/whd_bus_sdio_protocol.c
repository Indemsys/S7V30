...
static whd_result_t whd_bus_sdio_blhs(whd_driver_t whd_driver, whd_bus_blhs_stage_t stage);
  #endif
static whd_result_t whd_bus_sdio_download_resource(whd_driver_t whd_driver, whd_resource_type_t resource,whd_bool_t direct_resource, uint32_t address, uint32_t image_size);
...
whd_result_t whd_bus_sdio_wait_for_wlan_event(whd_driver_t whd_driver, cy_semaphore_t *transceive_semaphore)
{
  ...
  if ((whd_sdpcm_has_tx_packet(whd_driver) == WHD_TRUE) && (whd_sdpcm_get_available_credits(whd_driver) == 0))
  {
    ...
    result = cy_rtos_get_semaphore(transceive_semaphore, (uint32_t)MIN_OF(timeout_ms,WHD_THREAD_POKE_TIMEOUT), WHD_FALSE);
  }
  else
  {
    result = cy_rtos_get_semaphore(transceive_semaphore, (uint32_t)MIN_OF(timeout_ms,WHD_THREAD_POLL_TIMEOUT), WHD_FALSE);
  }
...
{
  whd_result_t retval;
  retval =whd_bus_transfer_bytes(whd_driver, BUS_WRITE, WLAN_FUNCTION, 0,(uint16_t)(whd_buffer_get_current_piece_size(whd_driver,buffer)- sizeof(whd_buffer_t)),(whd_transfer_bytes_packet_t *)(whd_buffer_get_current_piece_data_pointer(whd_driver,buffer)+sizeof(whd_buffer_t)));
...
whd_result_t whd_bus_sdio_init(whd_driver_t whd_driver)
{
  ...
  do
  {
    ...
    CHECK_RETURN(whd_bus_write_register_value(whd_driver, BUS_FUNCTION, SDIOD_CCCR_IOEN, (uint8_t)1,SDIO_FUNC_ENABLE_1));
    if (loop_count != 0)
    {
  ...
  if (whd_driver->bus_priv->sdio_config.sdio_1bit_mode == WHD_FALSE)
  {
    ...
    CHECK_RETURN(whd_bus_read_register_value(whd_driver, BUS_FUNCTION, SDIOD_CCCR_BICTRL, (uint8_t)1,&byte_data));
    CHECK_RETURN(whd_bus_write_register_value(whd_driver, BUS_FUNCTION, SDIOD_CCCR_BICTRL, (uint8_t)1,(byte_data & (~BUS_SD_DATA_WIDTH_MASK)) | BUS_SD_DATA_WIDTH_4BIT));
  ...
  loop_count = 0;
  while (((result = whd_bus_write_register_value(whd_driver, BUS_FUNCTION, SDIOD_CCCR_BLKSIZE_0, (uint8_t)1,(uint32_t)SDIO_64B_BLOCK)) == WHD_SUCCESS)
         && ((result = whd_bus_read_register_value(whd_driver, BUS_FUNCTION, SDIOD_CCCR_BLKSIZE_0, (uint8_t)1,&byte_data)) == WHD_SUCCESS)
  ...
  CHECK_RETURN(whd_bus_write_register_value(whd_driver, BUS_FUNCTION, SDIOD_CCCR_BLKSIZE_0,   (uint8_t)1,(uint32_t)SDIO_64B_BLOCK));
  CHECK_RETURN(whd_bus_write_register_value(whd_driver, BUS_FUNCTION, SDIOD_CCCR_F1BLKSIZE_0, (uint8_t)1,(uint32_t)SDIO_64B_BLOCK));
  CHECK_RETURN(whd_bus_write_register_value(whd_driver, BUS_FUNCTION, SDIOD_CCCR_F2BLKSIZE_0, (uint8_t)1,(uint32_t)SDIO_64B_BLOCK));
  CHECK_RETURN(whd_bus_write_register_value(whd_driver, BUS_FUNCTION, SDIOD_CCCR_F2BLKSIZE_1, (uint8_t)1,(uint32_t)0));                                                                                  /* Function 2 = 64 */
  ...
  CHECK_RETURN(whd_bus_write_register_value(whd_driver, BUS_FUNCTION, SDIOD_CCCR_INTEN, (uint8_t)1,INTR_CTL_MASTER_EN | INTR_CTL_FUNC1_EN | INTR_CTL_FUNC2_EN));
  ...
  if (whd_driver->bus_priv->sdio_config.high_speed_sdio_clock)
  {
    ...
    if ((byte_data & 0x1) != 0)
    {
      CHECK_RETURN(whd_bus_write_register_value(whd_driver, BUS_FUNCTION, SDIOD_CCCR_SPEED_CONTROL, 1,byte_data | SDIO_SPEED_EHS));
    }
    else
    {
  ...
  /* Wait till the backplane is ready */
  loop_count = 0;
  while (((result = whd_bus_read_register_value(whd_driver, BUS_FUNCTION, SDIOD_CCCR_IORDY, (uint8_t)1,&byte_data)) == WHD_SUCCESS)
  ...
  /* Set the ALP */
  CHECK_RETURN(whd_bus_write_register_value(whd_driver, BACKPLANE_FUNCTION, SDIO_CHIP_CLOCK_CSR, (uint8_t)1,(uint32_t)(SBSDIO_FORCE_HW_CLKREQ_OFF | SBSDIO_ALP_AVAIL_REQ | SBSDIO_FORCE_ALP)));

  loop_count = 0;
  while (((result = whd_bus_read_register_value(whd_driver, BACKPLANE_FUNCTION, SDIO_CHIP_CLOCK_CSR, (uint8_t)1,&byte_data)) == WHD_SUCCESS) && ((byte_data & SBSDIO_ALP_AVAIL) == 0) && (loop_count < (uint32_t)ALP_AVAIL_TIMEOUT_MS))
  {
    (void)cy_rtos_delay_milliseconds((uint32_t)1);   /* Ignore return - nothing can be done if it fails */
    loop_count++;
  }
  ...
  /* Enable F1 and F2 */
  CHECK_RETURN(whd_bus_write_register_value(whd_driver, BUS_FUNCTION, SDIOD_CCCR_IOEN, (uint8_t)1,SDIO_FUNC_ENABLE_1 | SDIO_FUNC_ENABLE_2));

  /* Setup host-wake signals */
  CHECK_RETURN(whd_bus_sdio_init_oob_intr(whd_driver));

  /* Enable F2 interrupt only */
  CHECK_RETURN(whd_bus_write_register_value(whd_driver, BUS_FUNCTION, SDIOD_CCCR_INTEN, (uint8_t)1,INTR_CTL_MASTER_EN | INTR_CTL_FUNC2_EN));
  ...
  CHECK_RETURN(whd_bus_read_register_value(whd_driver, BUS_FUNCTION, SDIOD_CCCR_BRCM_CARDCAP, (uint8_t)1,&byte_data));
  ...
  if ((byte_data & SDIOD_CCCR_BRCM_CARDCAP_CHIPID_PRESENT) != 0)
  {
    ...
    CHECK_RETURN(whd_bus_write_register_value(whd_driver, BACKPLANE_FUNCTION, SBSDIO_DEVICE_CTL,(uint8_t)1,(devctl | SBSDIO_DEVCTL_ADDR_RST)));

    CHECK_RETURN(whd_bus_sdio_read_register_value(whd_driver, BACKPLANE_FUNCTION, SBSDIO_FUNC1_SBADDRLOW, 1,&addrlow));
    CHECK_RETURN(whd_bus_sdio_read_register_value(whd_driver, BACKPLANE_FUNCTION, SBSDIO_FUNC1_SBADDRMID, 1,&addrmid));
    CHECK_RETURN(whd_bus_sdio_read_register_value(whd_driver, BACKPLANE_FUNCTION, SBSDIO_FUNC1_SBADDRHIGH, 1,&addrhigh));

    reg_addr =((addrlow << 8) | (addrmid << 16) | (addrhigh << 24))+ SDIO_CORE_CHIPID_REG;

    CHECK_RETURN(whd_bus_write_register_value(whd_driver, BACKPLANE_FUNCTION, SBSDIO_DEVICE_CTL,(uint8_t)1, devctl));
  ...
  CHECK_RETURN(whd_bus_read_register_value(whd_driver, BACKPLANE_FUNCTION, SBSDIO_FUNC1_SECURE_MODE, (uint8_t)1,&secure_chk));
  ...
  loop_count = 0;
  while (((result = whd_bus_read_register_value(whd_driver, BUS_FUNCTION, SDIOD_CCCR_IORDY, (uint8_t)1,&byte_data)) == WHD_SUCCESS) && ((byte_data & SDIO_FUNC_READY_2) == 0) && (loop_count < (uint32_t)F2_READY_TIMEOUT_MS))
  {
    (void)cy_rtos_delay_milliseconds((uint32_t)1);   /* Ignore return - nothing can be done if it fails */
    loop_count++;
  }
  ...
  CHECK_RETURN(result);
  #if (CYHAL_API_VERSION >= 2)
  cyhal_sdio_enable_event(whd_driver->bus_priv->sdio_obj, CYHAL_SDIO_CARD_INTERRUPT, CYHAL_ISR_PRIORITY_DEFAULT,WHD_TRUE);
  #else
...
whd_result_t whd_bus_sdio_deinit(whd_driver_t whd_driver)
{
  ...
  #if (CYHAL_API_VERSION >= 2)
  cyhal_sdio_enable_event(whd_driver->bus_priv->sdio_obj, CYHAL_SDIO_CARD_INTERRUPT, CYHAL_ISR_PRIORITY_DEFAULT,WHD_FALSE);
  #else
...
whd_bool_t whd_bus_sdio_wake_interrupt_present(whd_driver_t whd_driver)
{
  ...
  if (WHD_SUCCESS != whd_ensure_wlan_bus_is_up(whd_driver)) return WHD_FALSE;
...
uint32_t whd_bus_sdio_packet_available_to_read(whd_driver_t whd_driver)
{
  ...
  if ((I_HMB_HOST_INT & int_status) != 0)
  {
    ...
    if ((whd_bus_read_backplane_value(whd_driver,  SDIO_TO_HOST_MAILBOX_DATA(whd_driver), 4,
                                      (uint8_t *)&hmb_data) == WHD_SUCCESS) && (hmb_data > 0)) if (whd_bus_write_backplane_value(whd_driver, SDIO_TO_SB_MAILBOX(whd_driver), (uint8_t)4,
                                                                                                                                 SMB_INT_ACK) != WHD_SUCCESS) WPRINT_WHD_ERROR(("%s: Failed writing SMB_INT_ACK\n", __FUNCTION__));
...
whd_result_t whd_bus_sdio_reinit_stats(whd_driver_t whd_driver, whd_bool_t wake_from_firmware)
{
  ...
  /* Wait till the backplane is ready */
  loop_count = 0;
  while (((result = whd_bus_read_register_value(whd_driver, BUS_FUNCTION, SDIOD_CCCR_IORDY, (uint8_t)1,&byte_data)) == WHD_SUCCESS) &&((byte_data & SDIO_FUNC_READY_1) == 0) &&(loop_count < (uint32_t)F1_AVAIL_TIMEOUT_MS))
  {
    (void)cy_rtos_delay_milliseconds((uint32_t)1);   /* Ignore return - nothing can be done if it fails */
    loop_count++;
  }
...
{
  #if (CYHAL_API_VERSION >= 2)
  cyhal_sdio_enable_event(whd_driver->bus_priv->sdio_obj, CYHAL_SDIO_CARD_INTERRUPT, CYHAL_ISR_PRIORITY_DEFAULT,enable);
  #else
  ...
  #endif
{
  ...
  #if (CYHAL_API_VERSION >= 2)
  const cyhal_gpio_event_t expected_event =(config->is_falling_edge == WHD_TRUE)? CYHAL_GPIO_IRQ_FALL : CYHAL_GPIO_IRQ_RISE;
  #else
  const cyhal_gpio_irq_event_t expected_event =(config->is_falling_edge == WHD_TRUE)? CYHAL_GPIO_IRQ_FALL : CYHAL_GPIO_IRQ_RISE;
  #endif
...
static whd_result_t whd_bus_sdio_register_oob_intr(whd_driver_t whd_driver)
{
  ...
  #else
  cyhal_gpio_register_irq(config->host_oob_pin, config->intr_priority, whd_bus_sdio_oob_irq_handler,whd_driver);
  #endif
  return WHD_SUCCESS;
}
...
static whd_result_t whd_bus_sdio_enable_oob_intr(whd_driver_t whd_driver, whd_bool_t enable)
{
  const whd_oob_config_t *config =&whd_driver->bus_priv->sdio_config.oob_config;
  #if (CYHAL_API_VERSION >= 2)
  const cyhal_gpio_event_t event =(config->is_falling_edge == WHD_TRUE)? CYHAL_GPIO_IRQ_FALL : CYHAL_GPIO_IRQ_RISE;

  cyhal_gpio_enable_event(config->host_oob_pin, event, config->intr_priority,(enable == WHD_TRUE)? true : false);
  #else
  const cyhal_gpio_irq_event_t event =(config->is_falling_edge == WHD_TRUE)? CYHAL_GPIO_IRQ_FALL : CYHAL_GPIO_IRQ_RISE;
...
static whd_result_t whd_bus_sdio_init_oob_intr(whd_driver_t whd_driver)
{
  ...
  if (config->host_oob_pin == CYHAL_NC_PIN_VALUE) return WHD_SUCCESS;
  ...
  if (config->dev_gpio_sel) CHECK_RETURN(whd_bus_sdio_set_oob_interrupt(whd_driver, config->dev_gpio_sel));
...
exit:
  if (cmd_img) whd_mem_free(cmd_img);
  return WHD_SUCCESS;
}
