 ...
 */
static whd_result_t whd_download_wifi_clm_image(whd_interface_t ifp, const char *iovar, uint16_t flag, uint16_t dload_type, unsigned char *dload_buf, uint32_t len)
{
  whd_result_t     res;
  ...
  WPRINT_WHD_DATA_LOG(("FUNC: whd_download_wifi_clm_image\n"));
  ...
  res = whd_proto_set_iovar(ifp, buffer, NULL);
...
whd_result_t whd_process_clm_data(whd_interface_t ifp)
{
  ...
  WPRINT_WHD_DATA_LOG(("FUNC: whd_process_clm_data\n"));
  ...
  if ((chunk_buf = (unsigned char *)whd_mem_malloc(size2alloc)) != NULL)
  {
    ...
    for (i = 0; i < blocks_count; i++)
    {
      ...
      if (blocks_count != 1) transfer_progress = 0;

      for (j = 0; j < num_buff; j++)
      {
        if (size_read >= BLOCK_SIZE) chunk_len = BLOCK_SIZE;
        else chunk_len = size_read;
        ...
        ret = whd_download_wifi_clm_image(ifp, IOVAR_STR_CLMLOAD, dl_flag, DL_TYPE_CLM, chunk_buf,data_offset + chunk_len);
    ...
    if (ret != WHD_SUCCESS)
    {
      ...
      WPRINT_WHD_DEBUG(("clmload (%" PRIu32 " byte file) failed with return %" PRIu32 "; ", clm_blob_size,ret));
