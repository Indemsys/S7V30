...
uint8_t     thread_matlab_stack[THREAD_MATLAB_STACK_SIZE];
...
uint32_t Thread_MATLAB_create(void)
{
  ...
  if (ivar.en_matlab != BINARY_YES) return RES_ERROR;

  if (matlab_thread.tx_thread_id == TX_THREAD_ID) return RES_ERROR;


  res= tx_thread_create(&matlab_thread, "MATLAB", Thread_MATLAB,
                           0,
...
uint32_t Thread_MATLAB_terminate(void)
{
  ...
  NETLOG("MATLAB theread terminating status=%d", status);
  return status;
}
...
uint32_t Thread_MATLAB_delete(void)
{
  ...
  NETLOG("MATLAB theread deleting status=%d", status);
  return status;
}
...
uint32_t MATLAB_connection_server_delete(void)
{
  ...
  if (matlab_server_created)
  {
    ...
    NETLOG("MATLAB server deleting result: %04X", status);
...
static uint32_t _Do_Files_write_read_delete_test(void)
{
  ...
  if (tmp_buf == NULL)
  {
    NETLOG("Not enough memory for file buffer");
  ...
  if (f == NULL)
  {
    App_free(tmp_buf);
    tmp_buf = 0;
    NETLOG("Not enough memory for file descriptor");
    return FX_NOT_ENOUGH_MEMORY;
  }
  ...
  tx_thread_priority_change(tx_thread_identify(), THREAD_HIGHER_PRIO,&old_priority);
  do
  {
    ...
    {
      NETLOG("Recreating file for write error %d.", status);
      goto err;
    }
    ...
    if (status != FX_SUCCESS)
    {
      NETLOG("File write error %d.", status);
      goto err;
    }
    ...
    if (status != FX_SUCCESS)
    {
      NETLOG("File close error %d.", status);
      goto err;
    }
    ...
    if (matlab_cmd1.en_read_flag)
    {
      ...
      if (status != FX_SUCCESS)
      {
        NETLOG("Open file for read error %d.", status);
        goto err;
      }
      ...
      if (status != FX_SUCCESS)
      {
        NETLOG("File read error %d.", status);
        goto err;
      }
      ...
      {
        NETLOG("Readed file is different from the reference.");
        goto err;
      }
    ...
    if (matlab_cmd1.en_del_flag)
    {
      ...
      if (status != FX_SUCCESS)
      {
        NETLOG("File delete error %d.", status);
        goto err;
      }
    }
    else
  ...
  NETLOG("Test completed successfully");
  status = FX_SUCCESS;
err:
...
void _Do_Power_source_control(void)
{
  ...
  if (tmp_buf == NULL)
  {
    NETLOG("Not enough memory for string");
    return;
  }
