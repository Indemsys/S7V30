...
static whd_result_t whd_cdc_set_ioctl(whd_interface_t ifp, uint32_t command, whd_buffer_t send_buffer_hnd, whd_buffer_t *response_buffer_hnd)
{
  return whd_cdc_send_ioctl(ifp, CDC_SET, command, send_buffer_hnd, response_buffer_hnd);
}

static whd_result_t whd_cdc_get_ioctl(whd_interface_t ifp, uint32_t command, whd_buffer_t send_buffer_hnd, whd_buffer_t *response_buffer_hnd)
{
  return whd_cdc_send_ioctl(ifp, CDC_GET, command, send_buffer_hnd, response_buffer_hnd);
}

static whd_result_t whd_cdc_set_iovar(whd_interface_t ifp, whd_buffer_t send_buffer_hnd, whd_buffer_t *response_buffer_hnd)
{
  return whd_cdc_send_iovar(ifp, CDC_SET, send_buffer_hnd, response_buffer_hnd);
}

static whd_result_t whd_cdc_get_iovar(whd_interface_t ifp, whd_buffer_t send_buffer_hnd, whd_buffer_t *response_buffer_hnd)
{
 ...
 */
whd_result_t whd_cdc_send_ioctl(whd_interface_t ifp, cdc_command_type_t type, uint32_t command, whd_buffer_t send_buffer_hnd, whd_buffer_t *response_buffer_hnd)
{
  ...
  data_length =(uint32_t)(whd_buffer_get_current_piece_size(whd_driver,send_buffer_hnd)- sizeof(bus_common_header_t)-sizeof(cdc_header_t));
  ...
  if ((command == WLC_SET_VAR) || (command == WLC_GET_VAR))
  {
    ...
    if (data != ptr)
    {
      ...
      CHECK_RETURN(whd_buffer_set_size(whd_driver, send_buffer_hnd,(uint16_t)(data_length + sizeof(bus_common_header_t)+sizeof(cdc_header_t))));
    }
  }
  ...
  if ((flags & CDCF_IOC_ERROR) != 0)
  {
    ...
    if (status) return WHD_RESULT_CREATE((WLAN_ENUM_OFFSET - status));
    else return WHD_IOCTL_FAIL;
  }
 ...
 * @return    WHD result code
 */
whd_result_t whd_cdc_send_iovar(whd_interface_t ifp, cdc_command_type_t type, whd_buffer_t send_buffer_hnd, whd_buffer_t *response_buffer_hnd)
{
  if (type == CDC_SET)
  {
...
whd_result_t whd_cdc_tx_queue_data(whd_interface_t ifp, whd_buffer_t buffer)
{
  ...
  if (result != WHD_SUCCESS)
  {
    ...
    if (result != WHD_SUCCESS) WPRINT_WHD_ERROR(("buffer release failed in %s at %d \n", __func__, __LINE__));
...
whd_result_t whd_cdc_bdc_info_init(whd_driver_t whd_driver)
{
  ...
  if (cy_rtos_init_semaphore(&cdc_bdc_info->ioctl_mutex,"WHD_bdc_info_ioctl_mutex", 1, 0) != WHD_SUCCESS)
  {
  ...
  if (cy_rtos_init_semaphore(&cdc_bdc_info->ioctl_sleep,"WHD_bdc_info_ioctl_sleep", 1, 0) != WHD_SUCCESS)
  {
  ...
  if (cy_rtos_init_semaphore(&cdc_bdc_info->event_list_mutex,"WHD_bdc_info_event_list_mutex", 1, 0) != WHD_SUCCESS)
  {
  ...
  /* Create semaphore to protect event list management */
  if (cy_rtos_init_semaphore(&error_info->event_list_mutex,"WHD_dcd_errinf_event_list_mutex", 1, 0) != WHD_SUCCESS)
  {
    return WHD_SEMAPHORE_ERROR;
  }
...
void whd_process_cdc(whd_driver_t whd_driver, whd_buffer_t buffer)
{
      ...
      (id == cdc_bdc_info->requested_ioctl_id))
  {
    ...
    if (result != WHD_SUCCESS) WPRINT_WHD_ERROR(("Error setting semaphore in %s at %d \n", __func__, __LINE__));
...
void whd_process_bdc(whd_driver_t whd_driver, whd_buffer_t buffer)
{
  ...
  if (result != WHD_SUCCESS) WPRINT_WHD_ERROR(("%s failed at %d \n", __func__, __LINE__));
}
