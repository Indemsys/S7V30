...
extern "C"
{
  ...
  *                      Macros
  ******************************************************/
  #define WHD_PRINT_DISABLE   // Этим макросом запрещаем отладочный вывод несмотря на все последующие объявления
  ...
  ******************************************************/
  /* IF MFG TEST is enabled then disable all LOGGING VIA UART as
   ...
   + */
    ...
    #else
  extern void      WHD_RTT_LOGs(const char *fmt_ptr, ...);
      #define WPRINT_MACRO(args) do { WHD_RTT_LOGs args;} while (0 == 1)
    #endif
  #endif
