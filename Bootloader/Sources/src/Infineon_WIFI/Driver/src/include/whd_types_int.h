...
#define CHECK_IOCTL_BUFFER(buff)             if (buff == NULL){ WPRINT_WHD_ERROR( ("Buffer alloc failed in function %s at line %d \n", __func__, __LINE__) ); return WHD_BUFFER_ALLOC_FAIL; }
#define CHECK_PACKET_NULL(buff, err)         if (buff == NULL){ WPRINT_WHD_ERROR( ("No register function pointer in %s at line %d \n", __func__, __LINE__) ); return err;}
#define CHECK_PACKET_WITH_NULL_RETURN(buff)  if (buff == NULL){ WPRINT_WHD_ERROR( ( "No register function pointer in %s at line %d \n", __func__, __LINE__) ); return;}

#define CHECK_RETURN(expr)                       { whd_result_t check_res = (expr); if (check_res != WHD_SUCCESS) { WPRINT_WHD_ERROR( ("Function %s failed at line %d checkres = %u \n", __func__, __LINE__, (unsigned int)check_res) ); return check_res; } }
#define CHECK_RETURN_UNSUPPORTED_OK(expr)        { whd_result_t check_res = (expr); if (check_res != WHD_SUCCESS) { return check_res; } }
#define CHECK_RETURN_UNSUPPORTED_CONTINUE(expr)  { whd_result_t check_res = (expr); if (check_res != WHD_SUCCESS && check_res != WHD_WLAN_UNSUPPORTED) { return check_res; } }
#define RETURN_WITH_ASSERT(expr)                 { whd_result_t check_res = (expr); whd_assert("Command failed\n", check_res == WHD_SUCCESS); return check_res; }
#define CHECK_RETURN_IGNORE(expr)                { whd_result_t check_res = (expr); if (check_res != WHD_SUCCESS) { } }
