...
whd_result_t whd_msgbuf_txflow_init(whd_msgbuftx_info_t *msgtx_info)
{
    ...
    if (cy_rtos_init_semaphore(&msgtx_info->send_queue_mutex,"WHD_msgb_send_queue_mutex", 1, 0) != WHD_SUCCESS)
    {
        return WHD_SEMAPHORE_ERROR;
    }
...
static uint32_t whd_msgbuf_attach(struct whd_driver *whd_driver)
{
    ...
    if (cy_rtos_init_semaphore(&msgbuf->tx_pktids->pktid_mutex,"WHD_msgb_tx_pktid_mutex", 1, 0) != WHD_SUCCESS)
        goto fail;
    ...
    if (!msgbuf->rx_pktids)
        goto fail;
    /* Create the mutex protecting the packet send queue */
    if (cy_rtos_init_semaphore(&msgbuf->rx_pktids->pktid_mutex,"WHD_msgb_rx_pktid_mutex", 1, 0) != WHD_SUCCESS)
        goto fail;
...
whd_result_t whd_msgbuf_info_init(whd_driver_t whd_driver)
{
    ...
    /* Create the mutex protecting the packet send queue */
    if (cy_rtos_init_semaphore(&msgbuf_info->ioctl_mutex,"WHD_msgb_ioctl_mutex", 1, 0) != WHD_SUCCESS)
    {
        return WHD_SEMAPHORE_ERROR;
    }
    ...
    if (cy_rtos_init_semaphore(&msgbuf_info->ioctl_sleep,"WHD_msgb_ioctl_sleep", 1, 0) != WHD_SUCCESS)
    {
    ...
    if (cy_rtos_init_semaphore(&msgbuf_info->event_list_mutex,"WHD_msgb_event_list_mutex", 1, 0) != WHD_SUCCESS)
    {
        cy_rtos_deinit_semaphore(&msgbuf_info->ioctl_sleep);
    ...
    /* Create semaphore to protect event list management */
    if (cy_rtos_init_semaphore(&error_info->event_list_mutex,"WHD_msgb_erri_event_list_mutex", 1, 0) != WHD_SUCCESS)
    {
        return WHD_SEMAPHORE_ERROR;
    }
