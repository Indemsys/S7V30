...
{
  if (!whd_driver) return WHD_WLAN_ERROR;
...
{
  if (g_bt_whd_driver) return g_bt_whd_driver;
  else return NULL;
}
...
void whd_bus_bt_detach(whd_driver_t whd_driver)
{
  ...
  if (btdev)
  {
    if (btdev->bt_data) btdev->bt_data = NULL;
    if (btdev->bt_int_cb) btdev->bt_int_cb = NULL;
    if (whd_driver->bt_dev)
    {
...
whd_result_t whd_bus_download_resource(whd_driver_t whd_driver, whd_resource_type_t resource, whd_bool_t direct_resource, uint32_t address, uint32_t image_size)
{
