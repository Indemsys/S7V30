...
/*******************************************************************************************************************//**
 * @brief Check if the first entry (version number) in the factory flash is not set to 0 or 0xFFFFFFFF.
 *
 * @param[in]  p_table     Base address of the table to validate
 *
 * @retval  SSP_SUCCESS               FMI data table version exists.
 * @retval  SSP_ERR_INVALID_FMI_DATA  FMI data table version is not valid
 **********************************************************************************************************************/
__STATIC_INLINE ssp_err_t HW_FMI_HardwareTableVersionEntryCheck (uint32_t * p_table)
{
    /** The first index entry in the data table is the version number.  A value of 0 or 0xFFFFFFFF indicates invalid
     * factory flash. */
    if((0U != p_table[0]) && (0xFFFFFFFFU != p_table[0]))
    {
        return SSP_SUCCESS;
    }


    return SSP_ERR_INVALID_FMI_DATA;
}
