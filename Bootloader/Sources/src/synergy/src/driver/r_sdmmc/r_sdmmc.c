                           ...
                           sdmmc_io_write_mode_t const read_after_write)
{
    ...
    {
       SSP_ERROR_LOG(SSP_ERR_WRITE_FAILED, &g_module_name[0], &g_sdmmc_version);
    }
    return ret_val;
}
                              ...
                              sdmmc_io_address_mode_t  address_mode)
{
    ...
    uint16_t command     = SDMMC_CMD_IO_WRITE_EXT_SINGLE_BLOCK;  // Подготоавливаем слово для командного регистра SD_CMD модуля SDHI
    uint16_t byte_count  = 0U;
    uint32_t block_count = 0U;
    if (SDMMC_IO_MODE_TRANSFER_BLOCK == transfer_mode)
    {
    ...
    /** Configure the transfer interface for writing. */
    ret_val = r_sdmmc_transfer_write(p_ctrl, block_count, byte_count, p_source); // Здесь инициализируем DMA

    SDMMC_ERROR_RETURN(SSP_SUCCESS == ret_val, SSP_ERR_WRITE_FAILED);

    // Здесь подготавливаем содержимое для регистра аргумента модуля SDHI в соответствии с форматом команды IO_RW_EXTENDED из спецификации SDIO
    sdmmc_priv_sdio_arg_t argument = {0U};
    /* According to SDIO spec, 512 = 0. */
    argument.cmd_53_arg.count            = (count & SDMMC_SDIO_CMD52_CMD53_COUNT_MASK);
    argument.cmd_53_arg.function_number  = (function & SDMMC_SDIO_CMD52_CMD53_FUNCTION_MASK);
    ...
    ret_val = r_sdmmc_read_write_common(p_ctrl, block_count, byte_count, command, argument.arg); // Здесь отправляем команду с аргументом
    SDMMC_ERROR_RETURN(SSP_SUCCESS == ret_val, SSP_ERR_WRITE_FAILED);
...
static ssp_err_t r_sdmmc_bus_cfg (sdmmc_instance_ctrl_t * const p_ctrl)
{
    ...
    else
    {
        ...
        if (r_sdmmc_command_send(p_ctrl, SDMMC_CMD_SEND_CID, p_ctrl->status.sdhi_rca << 16)==true)
        {
            cid_reg.reg.sdrsp10 = p_ctrl->p_reg->SD_RSP10;
            cid_reg.reg.sdrsp32 = p_ctrl->p_reg->SD_RSP32;
            cid_reg.reg.sdrsp54 = p_ctrl->p_reg->SD_RSP54;
            cid_reg.reg.sdrsp76 = p_ctrl->p_reg->SD_RSP76;
        }
 ...
 * @param[in]     p_ctrl           Pointer to SDMMC instance control block.
 * @param[in]     block_count      Number of blocks to transfer.
 * @param[in]     block_sz_bytes   Bytes per block.
 * @param[in]     data_buff        Pointer to data to write to device.
 *
 * @retval         SSP_SUCCESS      Transfer successfully configured to write data.
 * @return                          See @ref Common_Error_Codes or functions called by this function for other possible
 *                                  return codes. This function calls:
                                ...
                                const uint8_t      * data_buff)
{
    ...
    // Если начало блока данных или количество данных кратно 4, то пересылки осуществлять по DMA из буфера источника в блочном режиме словами по 32-а бита
    // Иной размер  чем 32-бита FIFO модуля SDHI не принимает

    p_info->p_src          = data_buff;
    ...
    /* Round up to the nearest multiple of 4 bytes for the transfer. */
    uint32_t words         = (block_sz_bytes + (sizeof(uint32_t) - 1U)) / sizeof(uint32_t);
    p_info->length         = (uint16_t) words;  // Устанавливаем количество циклов  передачи по DMA в одном блоке

    R_BSP_IrqStatusClear(p_ctrl->transfer_irq);
    HW_SDMMC_DMAModeEnable(p_ctrl->p_reg, true);

    if ((0U != ((uint32_t) data_buff & 0x3U)) || (0U != (block_sz_bytes & 3U)))
    {
        // Если начало блока данных или количество данных не кратно 4, то пересылки осуществлять из промежуточного буфера

        p_info->irq           = TRANSFER_IRQ_EACH;            // Прерывания после каждого блока поскольку надо будет программно переносить данные в промежуточный буфер
        ...
        r_sdmmc_software_copy(data_buff, block_sz_bytes, (void *) &p_ctrl->aligned_buff[0]);
        p_info->p_src         = &p_ctrl->aligned_buff[0];

        p_ctrl->transfer_block_current = 1U;                                     // Счет начинаем с 1, поскольку первый блок уже скопирован
        p_ctrl->transfer_blocks_total  = block_count;                            //
        p_ctrl->p_transfer_data        = (uint8_t *) &data_buff[block_sz_bytes]; // Устанавливаем указатель на адрес следущего блока данных. Он будет использован при следующей пересылке в промежуточный буфер
        p_ctrl->transfer_dir           = SDMMC_TRANSFER_DIR_WRITE;
        p_ctrl->transfer_block_size    = block_sz_bytes;
    }

    cfg.activation_source      = HW_SDMMC_DmaEventGet((uint8_t) p_ctrl->hw.channel);

    /** Configure the transfer driver to write to the SD buffer. */
    ssp_ret_val = p_ctrl->p_lower_lvl_transfer->p_api->open(p_ctrl->p_lower_lvl_transfer->p_ctrl, &cfg); // Открываем канал DMA и активируем ожидание начала пересылки
