...
prog_file_name = "S7V30_bootloader.ewp"
...
def Create_IAR_groups_tree(current_path, xml_tree):
  ...
  for directory_object in dirs_iterator:
    if (
	not (
          directory_object.name.startswith('.')
          or directory_object.name.startswith("Doc")
          or directory_object.name.startswith("WEB_Сertificate_generation")          
	) 
...
for configuration_object  in xml_configuration_block:
  ...
  for dirpath, dirs, files in os.walk(proj_dir):
    if not (dirpath.startswith(proj_dir+"\\Out")
            ...
            or dirpath.startswith(proj_dir+"\\Doc")
            or dirpath.startswith(proj_dir+"\\Tools")
            or (dirpath == proj_dir)):
