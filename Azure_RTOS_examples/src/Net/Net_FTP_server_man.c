...
uint8_t                    ftp_server_stack_memory[FTP_SERVER_STACK_SIZE] BSP_PLACE_IN_SECTION_V2(".stack.ftp_server_thread")BSP_ALIGN_VARIABLE_V2(BSP_STACK_ALIGNMENT);
...
uint32_t Net_FTP_server_create(NX_IP  *ip_ptr, char *name)
{
  ...
  if (ivar.enable_ftp_server == 0) return RES_ERROR;
