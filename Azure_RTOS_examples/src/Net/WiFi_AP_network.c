...
uint8_t                 *wifi_ap_ip_stack_memory;
...
uint32_t WIFI_AP_init_TCP_stack(void)
{
  ...
  wifi_ap_ip_stack_memory           = App_malloc(WIFI_AP_IP_STACK_SIZE);
  wifi_ap_arp_memory                = App_malloc(NX_ARP_CACHE_SIZE);
  wifi_ap_dhcp_server_stack_memory  = App_malloc(WIFI_DHCP_SERVER_STACK_SIZE);


  if ((wifi_ap_ip_ptr == NULL) || (wifi_ap_ip_stack_memory == NULL) || (wifi_ap_arp_memory == NULL) || (wifi_ap_dhcp_server_stack_memory == NULL))  goto _err;
...
_err:
  ...
  App_free(wifi_ap_ip_stack_memory);
...
uint32_t WIFI_AP_start_network(void)
{
  ...
  MATLAB_connection_server_create(wifi_ap_ip_ptr);
