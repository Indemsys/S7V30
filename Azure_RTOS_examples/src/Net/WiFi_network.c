...
extern const char* WHD_event_to_string(whd_event_num_t val);
/*-----------------------------------------------------------------------------------------------------
...
{
  APPLOG("WIFI. Event: %d (%s) Reason: %d, Status: %d ", event_header->event_type, WHD_event_to_string((whd_event_num_t)event_header->event_type), event_header->reason, event_header->status);
...
static uint32_t WIFI_init_module(void)
{
  ...
  g_whd_sdio_cfg.sdio_1bit_mode                               = WHD_FALSE; // WHD_TRUE;
