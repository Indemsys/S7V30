...
uint8_t             telnet_server_stack_memory[TELNET_SERVER_STACK_SIZE] BSP_PLACE_IN_SECTION_V2(".stack.telnet_server_thread")BSP_ALIGN_VARIABLE_V2(BSP_STACK_ALIGNMENT);;
...
void Telnet_client_connect(NX_TELNET_SERVER *telnet_server_ptr, UINT logical_connection)
{
  ...
  if ((logical_connection == TELNET_LOGICAL_CONNECTION) && (telnet_server_ptr == &telnet_server))
  {
    ...
    {
      status = Task_VT100_create((ULONG)&mon_telnet_drv_driver,&telnet_vt100_drv_handle); // В контексте этого вызова будут выполнены функции драйвера init и deinit
      if (status == RES_OK)
      {
...
uint32_t TELNET_server_create(NX_IP *ip, NX_TELNET_SERVER  *telnet_server_ptr, char *name)
{
  ...
  if (NX_SUCCESS != err)
  {
    APPLOG("%s. Creation error %d",name, err);
    return RES_ERROR;
  }
  else
  {
    APPLOG("%s. Telnet server created", name);
  }
  ...
  if (NX_SUCCESS != err)
  {
    ...
    APPLOG("%s. Packet pool setting error %d",name, err);
    return RES_ERROR;
  }
  ...
  {
    nx_telnet_server_delete(telnet_server_ptr);
    APPLOG("%s. Starting error %d",name, err);
    return RES_ERROR;
  }
  else
  {
    APPLOG("%s. Started",name);
    return RES_OK;
  }
}
