...
//    if ((ed->ux_synergy_pipe_index == 2) || (ed->ux_synergy_pipe_index == 1))
//    {
...
//    }
    ...
    else
    {
        ...
        /* The toggle bit expected for the next transfer has to be memorized if the toggle is derived from the ED value.  */
        if (td -> ux_synergy_td_toggle ==  UX_SYNERGY_TD_TOGGLE_FROM_ED)
        {
            /* Read the PIPCxCTR register.  */
            synergy_register =  ux_hcd_synergy_register_read(hcd_synergy,UX_SYNERGY_HC_PIPE1CTR + ((ed -> ux_synergy_pipe_index - 1UL) * 2UL));
        ...
        ux_hcd_synergy_register_clear(hcd_synergy,  UX_SYNERGY_HC_PIPE1CTR +((ed -> ux_synergy_pipe_index - 1UL) * 2UL),(USHORT)UX_SYNERGY_HC_DCPCTR_PID_MASK);
        ...
        {
            /* This is an IN transfer.  We need to read the buffer into the transfer_request memory buffer.  */
            ux_hcd_synergy_buffer_read(hcd_synergy, ed);
