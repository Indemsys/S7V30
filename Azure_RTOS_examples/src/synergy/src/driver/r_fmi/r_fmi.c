...
/** "FMI" in ASCII, used to determine if channel is open. */
#define FMI_INITIALIZED              (0x00464D49ULL)
...
/* Length (in bytes) of part name to compare in order to validate the part name. */
#define FMI_PRIV_PRODUCT_NAME_COMPARE_LENGTH (4U)
...
/* Global variable keeping track of R_FMI_Init being called before any other r_fmi API. */
/*LDRA_INSPECTED 219 S User name does not start with an underscore. */
/*LDRA_INSPECTED 57 D The global variable cannot be assigned at the time of declaration. */
/*LDRA_INSPECTED 57 D */
static volatile uint32_t g_fmi_initialized BSP_PLACE_IN_SECTION_V2(".noinit");
...
static ssp_err_t R_FMI_Init (void)
{
    ...
    {
        /* Validate the first entry in the factory flash. */
        err = HW_FMI_HardwareTableVersionEntryCheck((uint32_t *) *R_FMIFRT);
        if (SSP_SUCCESS == err)
        {
    ...
    g_fmi_initialized = FMI_INITIALIZED;
    return SSP_SUCCESS;
}
 ...
 * @retval        SSP_ERR_ABORTED      r_fmi module not initialized before calling this API.
 **********************************************************************************************************************/
static ssp_err_t R_FMI_ProductInfoGet (fmi_product_info_t ** pp_product_info)
{
...
#endif
    FMI_ERROR_RETURN(FMI_INITIALIZED == g_fmi_initialized, SSP_ERR_ABORTED);
    fmi_header_t * product_info_address;

    product_info_address = HW_FMI_RecordLocate(*R_FMIFRT, FMI_PRODUCT_INFORMATION);
...
#endif
    FMI_ERROR_RETURN(FMI_INITIALIZED == g_fmi_initialized, SSP_ERR_ABORTED);
    fmi_priv_table_t const * p_table = *R_FMIFRT;

    /** Store unique ID in p_unique_id. */
    fmi_product_info_t * p_product_info = (fmi_product_info_t *)
            &p_table->data[p_table->offset[FMI_PRODUCT_INFORMATION]];
...
static ssp_err_t fmi_part_number_validate (void)
{
    ssp_err_t err = SSP_ERR_INVALID_FMI_DATA;
    fmi_priv_table_t const * p_reg = *R_FMIFRT;
    uint32_t const * p_product_name = &p_reg->data[FMI_PRIV_PRODUCT_NAME_INDEX_BEGIN];
    uint32_t const * p_ssp_product_name = &R_FMI_TABLE->data[FMI_PRIV_PRODUCT_NAME_INDEX_BEGIN];

    err = memcmp(p_ssp_product_name, p_product_name, FMI_PRIV_PRODUCT_NAME_COMPARE_LENGTH) ? SSP_ERR_INVALID_FMI_DATA : SSP_SUCCESS;
    return err;
}
