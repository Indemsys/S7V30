 ...
 * Macro definitions
 **********************************************************************************************************************/
#define SF_SPI_API_VERSION_MAJOR (2U)
#define SF_SPI_API_VERSION_MINOR (0U)
...
/** SPI framework control block.  Allocate an instance specific control block to pass into the
 * SPI framework API calls.
 * @par Implemented as
...
typedef struct st_sf_spi_api
{
    ...
    /** @brief  Lock the bus for a device. The locking allows devices to reserve a bus to themselves
    *           for a given period of time (i.e. between lock and unlock). This allows devices to complete several
    *           reads and writes on the bus without interrupt. The wait option allows thread to wait for the specified
    *           timeout when acquiring the bus mutex.
    * @par Implemented as
    *  - SF_SPI_LockWait()
    * @param[in]  p_ctrl      Pointer to the control block for the device.
