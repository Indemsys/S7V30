...
static const uint32_t crc16_precompiled_5935[256] =
{
...
static const uint32_t crc16_precompiled_755B[] =
{
...
/*-----------------------------------------------------------------------------------------------------
  ...
  \return uint16_t
-----------------------------------------------------------------------------------------------------*/
static uint16_t crc16_reflected(uint8_t *buf, uint32_t len, uint16_t crc_in, const uint32_t table[])
{
    /* we use uint32_ts, rather than uint16_ts, as they are likely to be
       faster. We just ignore the top 16 bits and let them do what they want.
  ...
  while (len-- != 0) crc16 = table[(crc16 ^*buf++) & 0xff] ^(crc16 >> 8);
...
{
  return crc16_reflected(buf,len,crc16_ccitt_start,crc16_ccitt_table_reverse)^ crc16_ccitt_xorout;
}
...
{
  return crc16_reflected(buf,len,seed,crc16_ccitt_table_reverse)^ crc16_ccitt_xorout;
}
