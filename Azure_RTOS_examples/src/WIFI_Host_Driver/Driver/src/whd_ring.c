...
whd_result_t whd_bus_m2m_sharedmem_init(whd_driver_t whd_driver)
{
    ...
    if (cy_rtos_init_semaphore(&whd_driver->host_suspend_mutex,"WHD_ring_host_suspend_mutex", 1, 0) != WHD_SUCCESS)
    {
    ...
    result = cy_rtos_init_event(&whd_driver->host_suspend_event_wait, "WHD_ring_event");
    if (result != WHD_SUCCESS)
    {
