...
char  *ini_fname[PARAMS_TYPES_NUM]        = { PARAMS_APP_INI_FILE_NAME        , PARAMS_MOD_INI_FILE_NAME        };
char  *ini_used_fname[PARAMS_TYPES_NUM]   = { PARAMS_APP_USED_INI_FILE_NAME   , PARAMS_MOD_USED_INI_FILE_NAME   };
char  *json_fname[PARAMS_TYPES_NUM]       = { PARAMS_APP_JSON_FILE_NAME       , PARAMS_MOD_JSON_FILE_NAME       };
char  *json_compr_fname[PARAMS_TYPES_NUM] = { PARAMS_APP_COMPR_JSON_FILE_NAME , PARAMS_MOD_COMPR_JSON_FILE_NAME };
...
static uint32_t Save_settings_buf_to_DataFlash(uint8_t *buf, uint32_t buf_sz, uint8_t ptype)
{
  ...
  if (S7G2_DataFlash_EraseArea(df_params_addr[ptype][0],DATAFLASH_PARAMS_AREA_SIZE) != RES_OK) goto EXIT_ON_ERROR;
  err_step = 4;
  ...
  if (S7G2_DataFlash_WriteArea(df_params_addr[ptype][0], tmp_buf, tmp_buf_sz) != RES_OK) goto EXIT_ON_ERROR;
  err_step = 5;
  // Стираем область памяти 2 хранения настроек
  if (S7G2_DataFlash_EraseArea(df_params_addr[ptype][1],DATAFLASH_PARAMS_AREA_SIZE) != RES_OK) goto EXIT_ON_ERROR;
  err_step = 6;
  ...
  if (S7G2_DataFlash_WriteArea(df_params_addr[ptype][1], tmp_buf, tmp_buf_sz) != RES_OK) goto EXIT_ON_ERROR;
...
uint32_t Check_settings_in_DataFlash(uint8_t ptype, T_settings_state *sstate)
{
  ...
  for (uint32_t i=0;i<2;i++)
  {
    ...
    // Читаем из  DataFlash размер блока данных
    flash_cbl.p_api->read(flash_cbl.p_ctrl, (uint8_t *)&sz  , flash_addr , 4);
    flash_cbl.p_api->read(flash_cbl.p_ctrl, (uint8_t *)&sstate->area_wr_cnt[i]  , flash_addr+4 , 4);
...
uint32_t Accept_certificates_from_file(void)
{
  ...
  if (S7G2_DataFlash_EraseArea(DATAFLASH_CA_CERT_ADDR,DATAFLASH_CA_CERT_AREA_SIZE) != RES_OK) goto EXIT_ON_ERROR;
  if (S7G2_DataFlash_WriteArea(DATAFLASH_CA_CERT_ADDR, data_buf, DATAFLASH_CA_CERT_AREA_SIZE) != RES_OK) goto EXIT_ON_ERROR;
