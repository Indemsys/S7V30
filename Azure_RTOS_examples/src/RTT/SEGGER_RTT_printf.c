...
void RTT_printf(unsigned buf_indx, const char *fmt, ...)
{
  unsigned int     n;
  va_list          ap;
  char            *rtt_str;

  rtt_str = App_malloc_pending(RTT_STR_SZ, 10);
  if (rtt_str != NULL)
  {
    va_start(ap, fmt);
    n = vsnprintf(rtt_str, RTT_STR_SZ, (const char *)fmt, ap);
    va_end(ap);
    SEGGER_RTT_Write(buf_indx, rtt_str, n);
    App_free(rtt_str);
  }
...
/*-----------------------------------------------------------------------------------------------------
  ...
  \param fmt
-----------------------------------------------------------------------------------------------------*/
void RTT_printf_str(unsigned buf_indx, const char *fmt, ...)
{
  unsigned int     n;
  va_list          ap;
  char            *rtt_str;
